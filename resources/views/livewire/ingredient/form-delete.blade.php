<div class="relative w-full p-4 h-full md:h-auto">
    <div>
        ...
        @if ($has_recipes)
            <div class="flex justify-center items-center gap-2 rounded-t">
                ...
                @if ($has_recipes)
                    <p class="text-red-500 font-bold">Existem receitas vinculadas a esse ingrediente.</p>
                @endif
            </div>
        @endif
    </div>
    <div class="relative p-4 text-center bg-white sm:p-5">
        <div class="flex justify-center items-center space-x-4">
            ...
            @if (!$has_recipes)
                <button wire:click="delete({{ $ingredient }})"
                    class="py-2 p       x-3 text-sm flex gap-1 items-center font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300">
