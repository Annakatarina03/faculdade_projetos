<div>
    <h3 class="text-sm font-medium text-gray-900">Receitas: </h3>
        ...
        @foreach ($revenues as $index => $revenue)
            <div>
                <input type="checkbox" id="revenue{{ $revenue->id }}" name="publish_revenues[]"
                    wire:model.lazy="publish_revenues" @checked (in_array($revenue->id, $publish_revenues = [])) wire:change="toogle"
                    value="{{ $revenue->id }}" class="hidden">
                <label for="revenue{{ $revenue->id }}"
                    class="flex items-center justify-between border-black border-2 rounded-3xl cursor-pointer label-checked:border-blue-600">
                    <div
